...
def get_id_token(request, firebase_uid=None):
    if not request.session.get('id_token'):
        ...
        custom_id_token: str = create_custom_id_token(firebase_uid=firebase_uid)
...
def get_refresh_token(request):
    return str(request.session.get('refresh_token'))


def create_custom_id_token(firebase_uid, developer_claims=None, app=None):
    return str(auth.create_custom_token(firebase_uid, developer_claims, app))
...
def exchange_refresh_token_for_id_token(request):
    ...
    id_token: str = str(response.json().get('id_token')) or None
...
def sign_in_with_email_and_password(request, email, password, to_redirect=False):
    ...
    id_token: str = response.json().get('idToken')
    refresh_token: str = response.json().get('refreshToken')
...
def get_firebase_user(firebase_uid=None, email=None, phone_number=None, id_token: str = None):
    if firebase_uid:
...
def send_email_verification(request, firebase_uid=None):
    ...
    payload = {
        ...
        'idToken': bytes(id_token).decode('UTF8')
    }
    print('payload ready')
    response = requests.post(url=url, json=payload)
    email = response.json().get('email') or None

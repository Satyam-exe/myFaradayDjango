...
def generate_url(uid, type_of_link):
    ...
    if type_of_link == 'email_verification':
        ...
        url = f'{settings.SITE_URL}auth/resetpassword/confirm/{code}'
...
def verify_code(code, type_of_code):
    code_object = URLCode.objects.get(pk=code) or None
    if not code_object:
        return None
    user = CustomUser.objects.get(pk=code_object.user) or None
    if not user:
        return None
    if code_object.type_of_code != type_of_code:
        return None
